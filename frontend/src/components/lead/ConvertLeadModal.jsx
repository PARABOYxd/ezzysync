...
export default function ConvertLeadModal({ open, onClose, lead, onConverted }) {
  const [form, setForm] = useState({ departure: '', members: 1, pricePerPerson: '' });
    ...
    <Modal open={open} onClose={onClose} title="Convert Lead to Booking" size="md">
      <form onSubmit={handleSubmit} className="space-y-5">
        <p className="text-sm text-slate-500">
          Creating a booking for <b className="text-slate-700">{lead.customerName}</b> ({lead.interest || 'Trip TBD'}).
        ...
        <FormRow>
          ...
          <Input label="Number of Travelers" icon={Users} type="number" min={1} value={form.members} onChange={set('members')} />
        </FormRow>
