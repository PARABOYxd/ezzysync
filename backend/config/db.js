...
async function ensureSchema() {
  ...
  try {
    ...
    // One chat per person, enforced by the database.
    //
    // Duplicates were not just a display problem - they were being created
    // constantly, because the ingest path does "SELECT a chat, and INSERT one
    // if there isn't one". Two messages arriving together both saw no chat and
    // both inserted, and nothing in the schema said that was wrong.
    //
    // phone_key is the identity we actually mean: the last ten digits, so
    // "9664029765" and "919664029765" collapse to the same value. Instagram
    // chats store a handle rather than a number, so anything without ten
    // digits keys on the lowercased text instead. Generated by Postgres, so it
    // cannot drift from `phone` and needs no application code to maintain.
    try {
      await query(`
      ...
      logger.warn({ err }, 'Note adding phone_key to whatsapp_chats');
    }

    // Merge chats that are the same person written two ways - "9664029765"
    // and "919664029765" - into one.
    //
    // Runs once, here at boot, and nowhere else. It briefly lived inside
    // listChats() too, which the inbox polls every three seconds, so an open
    // conversation could be deleted while an agent was reading it and the
    // thread would go blank until they reopened it from the list.
    //
    // The move and the delete run in one transaction because
    // whatsapp_messages.chat_id cascades on delete: as two loose statements, a
    // message arriving in the losing chat in between was deleted along with
    // it, permanently.
    try {
      const { rows: duplicates } = await query(`
        SELECT tenant_id, phone_key AS suffix,
      ...
      for (const group of duplicates) {
        ...
        try {
          ...
          await client.query(`DELETE FROM whatsapp_chats WHERE id = ANY($1::uuid[])`, [redundantIds]);
          await client.query('COMMIT');
          logger.info({ primaryId, redundantIds, suffix: group.suffix }, 'Merged duplicate whatsapp_chats');
        } catch (mergeErr) {
          await client.query('ROLLBACK').catch(() => {});
          // One bad group must not stop the rest, or the boot.
          logger.warn({ err: mergeErr, suffix: group.suffix }, 'Could not merge one duplicate chat group');
        } finally {
          client.release();
        }
      }
    ...
    // With the duplicates gone, make it impossible to create another. The
    // ingest path upserts against this constraint, so two messages arriving at
    // the same instant now produce one chat instead of racing each other.
    try {
      await query(`
        CREATE UNIQUE INDEX IF NOT EXISTS idx_whatsapp_chats_tenant_phone_key
        ON whatsapp_chats(tenant_id, phone_key);
      `);
    } catch (err) {
      logger.error({ err }, 'Could not enforce one chat per phone - duplicates may reappear');
    }
  } catch (err) {
