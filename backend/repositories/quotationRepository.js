...
async function insertQuotation(tenantId, quotationId, data, customerId) {
  const { rows } = await query(
    ...
    [
      ...
      JSON.stringify(data.departureDays || []),
      JSON.stringify(data.tripTypes || [])
    ]
  );
  return rows[0];
}
...
async function updateQuotation(tenantId, quotationId, merged) {
  const { rows } = await query(
    `UPDATE quotations SET
       ...
       departure_days = $22, trip_types = $23,
       updated_at = now()
    ...
    [
      ...
      JSON.stringify(merged.departureDays || []),
      JSON.stringify(merged.tripTypes || [])
    ]
  );
  return rows[0];
}
